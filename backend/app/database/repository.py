...
class Repository:
    ...
    def get_recent_source_articles(self, hours: int = 168) -> Dict[str, List[Any]]:
        cutoff_time = datetime.now(timezone.utc) - timedelta(hours=hours)
    ...
    def get_source_article(self, article_type: str, article_id: str) -> Optional[Any]:
        if article_type == "youtube":
            return self.session.query(YouTubeVideo).filter_by(video_id=article_id).first()
        if article_type == "openai":
            return self.session.query(OpenAIArticle).filter_by(guid=article_id).first()
        if article_type == "anthropic":
            return self.session.query(AnthropicArticle).filter_by(guid=article_id).first()
        return None
